...
from CM.CM_TUWdispatch.plot_bokeh import cmapping
#%% ===========================================================================
#
# =============================================================================
...
#     
# =============================================================================
def save_solution(solutions,path_download,name,cmap):
    tabs = plot_solutions(show_plot=False,solution=solutions,cmap=cmap)
...
# =============================================================================
#%%
def compare(scMapper,subScMapper,solutions,cmap,path2output=path2output):
...
# =============================================================================
    ...
    output_file(os.path.join(path2output,"output_scenarios_compare.html"),title="LinzAG-Scenarios")
    save(t)
    print("Done")    
#%%
def getSolutions(sc_names,sub_sc_names,path2output=path2output):
    print("Getting Data from <"+path2output+">...")
    solutions = {}
    for sc in sc_names:
        solutions[sc] = dict()
...
# =============================================================================
#     uncoment this for auto color mapping
# =============================================================================
#    cmap = cmapping(list(set([x for sheetname in ["Heat Generators","Heat Storage"] for t_sc in tec_scenarios for x in pd.read_excel(t_sc,sheetname).name.values.tolist()])))

# =============================================================================
#     define a custom color mapping
# =============================================================================
    bio = ["Bio HW","Bio KWK 2","Mitte Bio KWK"]
    abfall = ["Mitte RHKW"] 
    gas_kwk = ["Mitte Gud1A","Mitte Gud1B","Süd DT2","Süd GT 1-2-3"]
    hw = ["HW Dornach","HW Mitte","HW Süd"] 
    wp = ["AbwasserKanal WP","Donauwasser WP","Rauchgas WP"] 
    ab = ["AbwärmeDirektvoest","AbwärmeWPvoest"] 
    hs = ["heat storage"] 
    from bokeh.palettes import Greens,Oranges,RdPu,Reds,Blues,Purples,Greys
    cmap = customCmap([ (bio,Greens), 
                       (abfall,Oranges), 
                       (gas_kwk,RdPu), 
                       (hw,Reds), 
                       (wp,Blues), 
...
# =============================================================================
    ...
    for t_sc in tec_scenarios:
        ...
        for sub_sc in range(pd.read_excel(t_sc,"Default - External Data").
            ...
            print("+++++++++++++++++")
            subScenario_name = str(subScMapper.name[subScMapper.Prefix==str(sub_sc)].values[0])
            ...
            if check_solution(solutions):
                ...
                save_solution(solutions,path_download,name,cmap)
            else:
...
#%%
# =============================================================================
#     uncoment this for manual Loading of solver outputs for comparision chart 
# =============================================================================
#    dict_of_solutions = getSolutions(scMapper.name.values.tolist(),subScMapper.name.values.tolist())

# =============================================================================
#     comparision chart
# =============================================================================
    print("\n\n\nCreating Compare Figures...")
    compare(scMapper,subScMapper,dict_of_solutions,cmap)
