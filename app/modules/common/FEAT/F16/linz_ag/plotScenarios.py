...
from CM.CM_TUWdispatch.plot_bokeh import stack_chart,matrix,colormapping,costBarStack_chart,plotExtra_table,tab_panes,cmapping
# =============================================================================
#
# =============================================================================
...
def table(**kwargs):
    ...
    data_table = DataTable(source=source, columns=columns, width=3000, height=1000)
...
#                cmap = cmapping(dic["Technologies:"], path2cmap)
                ...
                p_i_1.plot_width = 1000
                p_i_1.plot_height = 500
                p_i_2.height = 500
                p_i_2.width = 1000
...
def pieTGM(cmap=-1,**kwargs):
    ...
    if cmap == -1:
        ...
        colors = [cmap[j] for i,j in enumerate(heatgeneartors)]
...
#     
# =============================================================================
def compareScnearioPlot(clu_flag=True,**kwargs):
    """
    Params:
        sc_names            list
    ...
    if kwargs["solutions"]:
        cost2 = costLineUp(solutions=kwargs["solutions"],sc_names=kwargs["sc_names"],sub_sc_names=kwargs["sub_sc_names"],cmap=kwargs["cmap"])
        results = table(solutions=kwargs["solutions"],sc_names=kwargs["sc_names"],sub_sc_names=kwargs["sub_sc_names"])
        stack = stackPlot(solutions=kwargs["solutions"],sc_names=kwargs["sc_names"],sub_sc_names=kwargs["sub_sc_names"],t=kwargs["t"],cmap=kwargs["cmap"])
        prices = pricePlots(solutions=kwargs["solutions"],sc_names=kwargs["sc_names"],sub_sc_names=kwargs["sub_sc_names"],t=kwargs["t"])
        if not clu_flag:
             tab_kwargs = {  "Levelized Cost of Energy (LCOE)":lcoe_2,
                        "LCOE -(Scenario VS Portfolio)":lcoe,
                        "Thermal Geneartion Mix":tgm,
                        "Results":results,
                        "Thermal Energy Mix":stack,
                        "Prices":prices,
                        "Cost Line Up (CLU)":cost2}
        else:
            tab_kwargs = {  "Levelized Cost of Energy (LCOE)":lcoe_2,
                        "LCOE -(Scenario VS Portfolio)":lcoe,
                        "Thermal Geneartion Mix":tgm,
                        ...
                        "Prices":prices,
                        "Cost Line Up (CLU)":cost2,
                        "CLU - (Szenario VS Portfolios)":cost,
                        **{"CLU - (Portfolio: " + sz+")":glyph for sz,glyph in cost3.items()}
                }
...
#%%    
    ...
    print("Done!")
    output_file("output_scenarios_compare.html",title="Dummy-Scenarios")
    show(t)